...
type TableProps = {
  ...
  loading: boolean;
  selectedItem: documentRecord | null;
...
export const Table: React.FC<TableProps> = ({ items, loading, selectedItem, setSelectedItem }) => {
  ...
  const tableHeaders = ["N", "Fecha", "Tipo", "Cargo", "Dependencia", "Destinatario", "Cargo Destinatario", "Dependencia Destinatario", "Archivo"];
              ...
              <tbody>
                {paginatedItems.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 lg:px-6 py-3">{new Date(item.fecha_emision).toLocaleDateString()}</td>
                    <td className="px-4 lg:px-6 py-3 uppercase">{tipo_docOp.find((option) => option.key === String(item.tipo_documento))?.value || "Desconocido"}</td>
                    ...
                    <td className="px-4 lg:px-6 py-3">{`${item.received.name} ${item.received.last_name}`}</td>
                    <td className="px-4 lg:px-6 py-3">{item.r_ucd.cargoDependencia.cargo.nombre}</td>
                    <td className="px-4 lg:px-6 py-3">{item.r_ucd.cargoDependencia.dependencia.nombre}</td>
