...
export const getDocumentos = async (id: string): Promise<{ success: boolean; message?: string; data?: Array<documentRecord> }> => {
  try {
...
export const createDocumento = async (id: string, data: ZDocumentS & { file_id: string }): Promise<{ success: boolean; message: string }> => {
  try {
...
export const updateDocumento = async (id: string, user_id: string, data: ZDocumentS & { file?: File | null; file_id?: string }): Promise<{ success: boolean; message: string }> => {
  try {
    const session = await auth();
    if (!session || !session?.user) throw new Error("No autorizado");
