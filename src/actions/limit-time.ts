...
export const limitTime = async (): Promise<{
  success: boolean;
  message?: string;
  ...
  try {
    ...
    if (edit_config && edit_config.fecha_inicio && edit_config.fecha_inicio > now) {
      modificationStart = edit_config.fecha_inicio;
    }

    let modificationEnd = user.modification_end_time;
    if (edit_config && edit_config.fecha_fin) {
      modificationEnd = new Date(Math.max(user.modification_end_time.getTime(), edit_config.fecha_fin.getTime()));
    }
