...
const SidebarMenuGroup: React.FC<SidebarMenuGroupProps> = ({ title, items, openMenu, setOpenMenu }) => {
  return (
    <nav className="mt-4">
      <h2 className="px-4 font-semibold text-text-section-title text-xs uppercase tracking-wide">{title}</h2>
...
const SidebarMenuItem: React.FC<{
  ...
  const parentItemClasses = `
    ...
    ${isActive || isSubmenuActive ? "bg-link-active text-link-main" : "text-text-link hover:bg-link-hover"}
  `;
      ...
      {hasSubmenus ? (
        <>
          <div onClick={toggleSubmenu} className={parentItemClasses}>
            ...
            <div className="flex items-center space-x-2">
              {item.badge && <span className="inline-block bg-link-main p-1 px-2 rounded-full text-white text-xs">{item.badge}</span>}
            ...
            <ul className="space-y-1 mt-1 ml-4">
              {item.submenus!.map((sub, subIndex) => {
                ...
                const subItemClasses = `
                  ...
                  ${isSubItemActive ? "text-link-main" : "text-text-link hover:bg-link-hover"}
                `;
                return (
                  <li key={subIndex}>
        ...
        <Link
          ...
          className={`
            ...
            ${isActive ? "bg-link-active text-text-main" : "text-text-link hover:bg-link-hover"}
          `}
        >
          <div className="flex items-center gap-4">
            {item.icon && <span>{item.icon}</span>}
