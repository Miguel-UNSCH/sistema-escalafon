...
export const Pagination = ({ currentPage, totalPages, setCurrentPage }: { currentPage: number; totalPages: number; setCurrentPage: Dispatch<SetStateAction<number>> }) => {
  return (
    <nav className="flex flex-wrap justify-between items-center p-4">
      <span className="flex gap-2 text-subtext0 text-sm">
        ...
        <span className="font-semibold text-subtext1">
          {currentPage * 10 - 10 + 1}-{Math.min(currentPage * 10, 1400)}
        </span>{" "}
        de <span className="font-semibold text-subtext1">{1400}</span>
      </span>
      ...
      <ul className="inline-flex -space-x-px h-8 text-sm">
        ...
        <li>
          <button onClick={() => setCurrentPage(1)} className={`px-3 h-8 flex items-center justify-center ${currentPage === 1 ? "text-red bg-mantle" : "bg-base hover:bg-crust"}`}>
            1
          </button>
        </li>
